...
class BotAssistant:
    ...
    def checkGuess(self, guess):
        ...
        if answer > guess:
          ...
          self.hint = self.HIGHER_HINT
          self.previousGuess = guess
          if previousHint and previousHint == self.HIGHER_HINT and diff <= 2:
            # Very close to number
            return self.SSML_SPEAK_START + self.STEAM_AUDIO + \
              BotAssistant.getRandomPrompt(self.REALLY_HOT_HIGH_PROMPTS) + SSML_SPEAK_END
          else:
          ...
          self.hint = self.LOWER_HINT
          self.previousGuess = guess
          if previousHint and previousHint == self.LOWER_HINT and diff <= 2:
            # Very close to number
            return self.SSML_SPEAK_START + self.STEAM_AUDIO + \
